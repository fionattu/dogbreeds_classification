...
def main():
    ...
    with open(data_dir+'x_inception_fc_data.pickle', 'rb') as handle:
        load_data = pickle.load(handle)

    logreg = LogisticRegression(multi_class='multinomial', solver='lbfgs', random_state=SEED)
    logreg.fit(load_data['train_x'], (load_data['train_y'] * range(num_classes)).sum(axis=1))
    valid_probs = logreg.predict_proba(load_data['test_x'])
    ...
    reorder_df.to_csv("./test_submit.csv", index=False)
