...
# Method:   1. ensemble features of xception and inception(Kaggle rank: 494 score:0.303)
#         =>2. ensemble predicted probs of xception and inception(Kaggle rank: 465 score:0.286)
...
def main():
    ...
    # Ensemble 2: average predict probs
    train_x_bf = load_data['train_x'][:,0:2048]
    train_i_bf = load_data['train_x'][:, 2048:]
    test_x_bf = load_data['test_x'][:,0:2048]
    test_i_bf = load_data['test_x'][:, 2048:]
    ytr = load_data['train_y']
    logreg1 = LogisticRegression(multi_class='multinomial', solver='lbfgs', random_state=SEED)
    logreg1.fit(train_x_bf, (ytr * range(load_data['num_class'])).sum(axis=1))
    probs1 = logreg1.predict_proba(test_x_bf)
    logreg2 = LogisticRegression(multi_class='multinomial', solver='lbfgs', random_state=SEED)
    logreg2.fit(train_i_bf, (ytr * range(load_data['num_class'])).sum(axis=1))
    prob2 = logreg2.predict_proba(test_i_bf)
    # without formatting
    df1 = {'id': load_data['test_id']}
    res1 = pd.DataFrame(data=df1)
    res2 = pd.DataFrame(columns=load_data['selected_labels'], data=(probs1+prob2)/2)
    ...
    reorder_df.to_csv("./ave_xiception_submit.csv", index=False)
